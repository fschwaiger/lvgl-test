...
static lv_obj_t *av_create_arc_dial(lv_obj_t *parent, lv_coord_t x, lv_coord_t y)
{
...
static lv_obj_t *av_create_label(lv_obj_t *parent, lv_coord_t x, lv_coord_t y)
{
...
static void av_create_tick(lv_obj_t *parent, lv_coord_t x, lv_coord_t y, int angle, lv_color_t color)
{
...
void av_create_rpm_widget(lv_obj_t *parent, lv_coord_t x, lv_coord_t y, lv_subject_t *pSubject)
{
    lv_obj_t *arc = av_create_arc_dial(parent, x, y);
    lv_arc_bind_value(arc, pSubject);
    lv_subject_add_observer(pSubject, av_bind_color, arc);

    lv_obj_t *label = av_create_label(parent, x + 64, y + 70);
    lv_label_bind_text(label, pSubject, "%.0f");

    av_create_tick(parent, x, y, 340, AMBER);
    av_create_tick(parent, x, y, 350, RED);
    for (int angle = 135; angle <= 360; angle += 45)
    {
        av_create_tick(parent, x, y, angle, WHITE);
    }
}
